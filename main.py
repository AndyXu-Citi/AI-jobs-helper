...
class AIPipeline:
    ...
    async def stage2_collect(self, max_count=5):
        ...
        try:
            ...
            cursor.execute(
                "SELECT url, source_type FROM task_queue WHERE status='PENDING' LIMIT %s",
                (max_count,),
            )
            pending = [(row["url"], row["source_type"]) for row in cursor.fetchall()]
        finally:
            conn.close()
    ...
    def stage3_process(self, max_count=5):
        ...
        try:
            ...
            cursor.execute(
                ...
                "WHERE t.status='COLLECTED' LIMIT %s",
                (max_count,),
            )
            tasks = [(row["url"], row["source_type"], row["markdown_text"]) for row in cursor.fetchall()]
        finally:
            conn.close()
...
def show_results():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute(
            "SELECT url, structured_json, processed_at FROM final_results "
    ...
    for row in rows:
        print(f"\nURL: {row['url']}")
        print(f"Completed: {row['processed_at']}")
        print(f"Data:\n{row['structured_json']}")
        print("-" * 60)
