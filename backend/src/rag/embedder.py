...
class HuggingFaceEmbedder:
    """基于本地 HuggingFace bge-m3 的 Embedding 封装（sentence-transformers）。

    与 OllamaEmbedder 提供完全一致的接口（embed_one / embed_many / dim），
    区别是模型在 Python 进程内由 sentence-transformers 直接加载，
    不需要额外运行 Ollama 服务。适合已经习惯用 HF 管理模型的场景。
