...
- query_rag(question, top_k)               bge-m3 语义检索（Milvus + HuggingFace）
        ...
        results.append(
            {
                ...
                "detail_labels": boss.get("detail_labels") or [],
...
def query_rag(question: str, top_k: int = 5) -> str:
    ...
    依赖本地 sentence-transformers 加载的 bge-m3（HuggingFaceEmbedder，进程内，无需 Ollama）和已建好的
...
def get_skill_gap(top_n: int = 10) -> str:
    ...
    # 把 MySQL 里全部 boss_zhipin 岗位拍成 JobRecord
    ...
    # 市场总热度（不区分 have/learning），直接用 Boss API 返回的 skills 标签
    # 与 /api/report / compute_skill_gap 口径一致，不再用硬编码词典 + 子串匹配。
    market_counter: dict[str, int] = {}
    for row in rows:
        for sk in (row.get("skills") or []):
            if sk and isinstance(sk, str) and sk.strip():
                market_counter[sk.strip()] = market_counter.get(sk.strip(), 0) + 1
        for sk in (row.get("detail_labels") or []):
            if sk and isinstance(sk, str) and sk.strip():
                market_counter[sk.strip()] = market_counter.get(sk.strip(), 0) + 1
    market_top = sorted(
