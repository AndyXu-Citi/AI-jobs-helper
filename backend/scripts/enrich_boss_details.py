...
def load_boss_jobs_needing_enrichment(
    ...
    for row in rows:
        try:
            ...
            logger.warning(f"[parse] id={row['id']} structured_json 损坏，跳过")
            continue

        # 不重跑：已有 post_description 就跳过
        if not rerun and structured.get("_boss", {}).get("post_description"):
            continue
        out.append((row["id"], row["url"], structured))
...
async def enrich(limit: int | None, rerun: bool):
    ...
    if skipped_missing_ids:
        logger.warning(
            f"[警告] {skipped_missing_ids} 条岗位缺 securityId/lid（旧数据没保存），"
            "这些条目此次无法富化"
        )
    ...
    print()
    print("=" * 70)
    print(f"[完成] 富化完成：{updated} 条 Boss 岗位补上了 JD 正文 + 公司 + 学历经验")
    print("=" * 70)
    print()
    print("下一步：")
