...
# Windows GBK 控制台无法编码 emoji / 特殊符号，打印 LLM 生成内容（含 🎯✅ 等）
# 时易抛 UnicodeEncodeError 中断。改为遇到不可编码字符用 '?' 替换而非崩溃。
try:
...
def main():
    ...
    if not args.no_record:
        try:
            ...
            print(f"[已记录] 已记录到 agent_runs.db (id={run_id}, status={status_tag}){tip}")
        except Exception as e:
            # 记录失败不应该让 Agent 本身的输出受影响
            print(f"[警告] 记录到 agent_runs.db 失败: {e}")
